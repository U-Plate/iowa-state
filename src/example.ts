...
// ex: 243780
const SCHOOL_GOV_CODE = 0; // TODO: replace with your school's id
...
class Example extends School {
  ...
  private async fetchAndParseHall(loc: string, date: string): Promise<HallData> {
    ...
    const rawMeals: any[] = []; // TODO: pull the per-meal-time menu out of your response
    ...
    for (const meal of rawMeals) {
      ...
      mealTimeHours[mealName] = JSON.stringify({ Start: meal.startTime, End: meal.endTime });
      meals[mealName] ??= [];
